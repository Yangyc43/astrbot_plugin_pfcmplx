...
- 数据以 **`data_new.json` 为主**（共 403 件武器）；文件缺失或损坏时自动回退到 `data.json`、`data2.json`
- 找不到武器时会明确提示，不会静默无响应
...
2. 确保以下文件**完整存在**：
   ...
   - `main.py`
   - `metadata.yaml`
   - `data_new.json`（主要数据，403 件武器）

3. 在 AstrBot 后台或插件管理页中重载 / 重启插件。

4. 若修改过代码，直接替换文件后重新加载即可。

## 数据文件说明

| 文件 | 内容 | 优先级 |
| --- | --- | --- |
| `data_new.json` | 完整武器数据库（403 件） | 1（优先） |
| `data.json` | 精简示例数据（6 件） | 2（回退） |
| `data2.json` | 精简示例数据（10 件） | 3（回退） |

## 技术说明

- 插件基于 AstrBot `Star` API 开发
- 入口：`main.py`
- 使用 `@filter.command("request")` 注册指令，通过函数签名注入参数 `target`
- 数据文件加载顺序固定为 `data_new.json` → `data.json` → `data2.json`
